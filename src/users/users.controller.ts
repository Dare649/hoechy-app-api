...
export class UsersController {
    ...
    @Put('update_user/:id')
    @Roles(Role.ADMIN, Role.DRIVER, Role.EMPLOYEE)
    @ApiOperation({
    ...
    @Put(':id/deactivate_user')
    @Roles(Role.ADMIN)
    @ApiOperation({
        summary: 'This api allows the admin user to deactivate an existing user'
    })
    async deactivate_user(
        @Param('id') id: string,
    ): Promise<any> {
        try {
            return await this.user_service.deactivate_user(id);
        } catch (error) {
            throw new BadRequestException(`Error deactivating user ${error.message}`);
        }
    }


    @Put(':id/activate_user')
    @Roles(Role.ADMIN)
    @ApiOperation({
        summary: 'This api allows the admin user to activate an existing user deactivated user'
    })
    async activate_user(
        @Param('id') id: string,
    ): Promise<any> {
        try {
            return await this.user_service.activate_user(id);
        } catch (error) {
            throw new BadRequestException(`Error activating user ${error.message}`);
        }
    }
